...
# Function to select specialization. Provide google cloud project id (https://support.google.com/googleapi/answer/7014113?hl=en), computing location (https://cloud.google.com/vertex-ai/docs/general/locations) and task prompt.
# Returns selected specialization as string or "Error: No suitable spec found" if no suitable specialization found
def select_specialization(project_id: str, location: str, prompt: str) -> str:
    ...
    prompt = (
        f"Задача: \"{prompt}\"\nИз следующего списка специализаций разнорабочего выбери ту, которая лучше всего "
        f"описывает задачу, и в качестве ответа дай только одну выбранную специализацию. Если ни одна специализация "
        f"не подходит под задачу напиши \"Error: No suitable spec found\".\nСписок специализаций:\n")
